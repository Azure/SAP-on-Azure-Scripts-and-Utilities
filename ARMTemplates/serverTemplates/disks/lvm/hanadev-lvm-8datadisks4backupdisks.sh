...
# Update fstab
echo "/dev/sdk1 /hana/shared  ext4  defaults  0  2" | sudo tee -a /etc/fstab

# Creating the /usr/sap volume
sudo parted /dev/sdl --script mklabel gpt mkpart ext4part ext4 0% 100%
partprobe /dev/sdl1

sudo mkdir /usr/sap
# Update fstab
echo "/dev/sdl1 /usr/sap  ext4  defaults  0  2" | sudo tee -a /etc/fstab
