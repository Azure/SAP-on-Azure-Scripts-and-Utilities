...
sudo parted /dev/sdi --script mklabel gpt mkpart ext4part ext4 0% 100%
partprobe /dev/sdi1

sudo mkdir /hana/shared
# Update fstab
echo "/dev/sdi1 /hana/shared  ext4  defaults  0  2" | sudo tee -a /etc/fstab
...
sudo mkdir /usr/sap
# Update fstab
echo "/dev/sdj1 /usr/sap  ext4  defaults  0  2" | sudo tee -a /etc/fstab

# Creating the /hana/backup volume
sudo pvcreate /dev/sdk
sudo pvcreate /dev/sdl

sudo vgcreate backup-vg01 /dev/sdk /dev/sdl
